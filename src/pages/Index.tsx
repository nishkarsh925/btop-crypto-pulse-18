...
const Index = () => {
  const { cryptoData, isLoading, error, lastUpdate, isConnected, toggleFavorite, refetch } = useCryptoData();
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = cryptoData.filter(
      (crypto) =>
        ...
        crypto.pair.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 w-64 bg-slate-800/50 border-slate-600 text-white placeholder-slate-400 focus:border-cyan-500 focus:ring-cyan-500/20"
                />
              </div>
              ...
              <div className="flex items-center space-x-2">
                {isConnected && !isLoading && !error ? (
                  <div className="flex items-center space-x-1" title="Connected to Binance">
                    <Wifi className="w-4 h-4 text-green-400" />
                    <span className="text-xs text-green-400 hidden sm:inline">BINANCE</span>
                  </div>
                ) : isLoading ? (
                  <div className="flex items-center space-x-1" title="Connecting to Binance...">
                    <div className="w-4 h-4 border-2 border-yellow-400 border-t-transparent rounded-full animate-spin" />
                    <span className="text-xs text-yellow-400 hidden sm:inline">SYNC</span>
                  </div>
                ) : (
                  <div className="flex items-center space-x-1" title="Disconnected from Binance">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          {lastUpdate && (
            <div className="text-center mb-6">
              <p className="text-slate-400 text-sm">
                Last updated from Binance: {lastUpdate.toLocaleTimeString()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {featuredCoins.map((coin) => (
              <div key={coin.id} className="bg-slate-800/30 backdrop-blur-lg rounded-2xl border border-slate-700/50 p-6 hover:bg-slate-800/50 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-cyan-500/10">
          ...
          <div className="bg-slate-800/30 backdrop-blur-lg rounded-2xl border border-slate-700/50 overflow-hidden">
            <div className="p-6 border-b border-slate-700/50">
              ...
              <p className="text-slate-400 mt-1">Trade the world's leading cryptocurrencies - Live data from Binance</p>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-slate-800/50">
                  <tr>
                    ...
                    <th 
                      className="text-left py-4 px-6 text-slate-300 font-semibold cursor-pointer hover:text-cyan-400 transition-colors"
                  ...
                  {filteredAndSortedData.map((crypto, index) => (
                    ...
                    >
                      <td className="py-4 px-6">
                        <button
                          onClick={() => handleFavoriteToggle(crypto.id)}
                          className="text-slate-400 hover:text-yellow-400 transition-colors"
                        >
                      ...
                      <td className="py-4 px-6">
                        <Link to={`/trade/${crypto.symbol.toLowerCase()}`}>
                          <Button 
                            size="sm" 
                            className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white border-0 shadow-lg hover:shadow-cyan-500/25 transition-all duration-200 group-hover:scale-105"
                          >
                            Trade
